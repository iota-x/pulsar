 ...
 */
export const initiateCustomAction: ActionHandler = async (config, triggerData) => {
  const instruction = ((config.instruction as string) || '').toLowerCase();

  switch (instruction) {
    case 'swap':
      return executeBuySellOrder(config, triggerData);
    case 'stake':
    case 'unstake':
      return stakeUnstake({ ...config, mode: instruction }, triggerData);
    case 'send':
    case 'transfer':
      return sendTokens(config, triggerData);
    default: {
